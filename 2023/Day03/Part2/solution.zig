...
pub fn main() !void {
    ...
    while (lineIterator.next()) |line| {
        ...
        for (line, 0..) |char, charX| {
            ...
            var nearbyPartNumbers = std.ArrayList(u32).init(allocator);
            ...
            for (partNumbers.items) |partNumber| {
                ...
                try nearbyPartNumbers.append(partNumber.value);
            }

            if (char == '*' and nearbyPartNumbers.items.len == 2) {
                const multipliedValue = nearbyPartNumbers.items[0] * nearbyPartNumbers.items[1];
                try stdout.print("Symbol \"{c}\" at {d},{d} has two neighbours, adding {d}\n", .{
                    char, charX, lineY, multipliedValue
                });
